...
static void print_help( const char* prog_name )
{
  printf( 
   ...
   "  -c <on/off>, --crc4=<on/off> Enables of disables CRC-4 check\n"
   "       Valid only with framing=16.\n"
   "       Default = on.\n"
   "  -n <num-of-frames>, --num-of-frames=<integer> Sets number of frames\n"
   "       to receive. Default value is 0 (unlimited). \n" 
   "  -t <tsnum:file>, --timeslot=<tsnum:file> Sets a file for a \n"
   "       specified timeslot. File will be written each time timeslot\n"
   "       data is received. Example: -t 1:/tmp/ts1\n"
      ...
      case 't': // we assume something like --timeslot=1:/dev/urandom
        {
          ...
          if ( ts >= E1_TS_NUM ) {
            fprintf( stderr,
                     "timeslot number %d is too big (max = 31)\n", ts );
            free( tofree );
            return EINVAL;
          }
          e1_demux_set_file_for_timeslot( ts, optarg );
          free( tofree );
        }
        break;
...
//                        V  V
    ...
    c = getopt_long( argc, argv, "n:F:c:vhVt:",
                     long_options, NULL );
    if( c == -1 ) {
      break;
    } else {
  ...
  if ( p->verbose ) {
    ...
    if ( p->framing != E1_FRAMING_DOUBLEFRAME ) {
      fprintf( stderr, "CRC-4 bits checking is %s\n", 
                     p->crc4 ? "on" : "off" );
    }
    
    if ( p->max_frames > 0 ) {
      fprintf( stderr, "will receive %d frames\n", p->max_frames );
    }
  }
